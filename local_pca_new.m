...
function [idx, infos] = ...
    local_pca_new(X, k, stop_rule, input, opt)
...
while ((convergence == 0) && (iter < iter_max))
    ...
    if ((VQ==2 && iter == 0))
        ...
        eps_rec_clust = zeros(k,1);
        for j = 1 : k

            if cent_crit == 1
                diff = (scal_X(idx==j) - mean(scal_X(idx==j)) - rec_data{j});
            else
                diff = scal_X(idx==j) - rec_data{j};
            end

            eps_rec_clust(j) = mean(sum(diff.^2));
