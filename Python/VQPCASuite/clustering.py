...
class MBvqpca:
    ...
    def __init__(self, X, itmax=200, atol=1e-8, rtol=1e-8, ctol=1e-6, batch_size=32):
        ...
        # Parameters for convergence
        self.itmax_ = itmax
        self.atol_  = atol  # Tolerance on reconstruction error variance
        self.rtol_  = rtol  # Tolerance on reconstruction error
        self.ctol_  = ctol  # Tolerance on centroids
    ...
    def initialize_centroids(self, method, Ci=None):
        ...
        if Ci == None:
            if method == 'kmeans':
                # Centroids are initialized using k-means
                kmeans = KMeans(n_clusters=self.k_, random_state=0)
                # Select a random batch of data points
                batch = self.X_[np.random.choice(self.X_.shape[0], self.batch_size_, replace=False)]
                kmeans.fit(batch)
                self.labels_ = kmeans.labels_
                self.C_ = kmeans.cluster_centers_
    ...
    def fit(self, k=2, q=0.99, n_start=2, verbose=True, init_centroids='kmeans'):
        ...
        # Initialize centroids
        avail_methods = ['kmeans', 'random', 'uniform']
        if init_centroids not in avail_methods:
            init_centroids = 'kmeans'
            print("\n You need to specify one between ", avail_methods)
            raise Warning("Specified initialization method does not exist, kmeans will be used")
        ...
        while conv == False and iter < self.itmax_:
            ...
            batch = self.X_[np.random.choice(self.X_.shape[0], self.batch_size_, replace=False)]
            # Matrix of projection errors
            eps_matrix = np.zeros((batch.shape[0], self.k_))

            # Perform PCA in each cluster and calculate projection error
            for i in range(self.k_):
                # Predict the point using the local PCA model
                U_scores = self.pca_[i].transform(batch)
                # Reconstruct X
                X_rec    = self.pca_[i].inverse_transform(U_scores)
                # Calculate norm of the residual
                eps_matrix[:,i] = np.sum((batch - X_rec)**2, axis=1)
            ...
            centroids_new = np.zeros((self.k_, self.X_.shape[1]))
            for i in range(self.k_):
                centroids_new[i,:] = np.mean(batch[labels_new==i,:], axis=0)
            ...
            for i in range(self.k_):
                pca = PCA(n_components=self.q_)
                pca.fit(batch[labels_new==i,:])
                pca_list.append(pca)
                U_local = pca.transform(batch[labels_new==i])
                X_local_rec = pca.inverse_transform(U_local)
                rec_err_clust.append(mean_squared_error(batch[labels_new==i], X_local_rec))
    ...
    def write_output_files(self, foldername=None, format='h5'):
        ...
        if foldername == None:
            # Get the current date
            current_date = datetime.now().strftime("%Y-%m-%d")
            foldername = "MiniBatchVQPCA_k" + str(self.k_) + "q_" + str(self.q_) + '_' + current_date
